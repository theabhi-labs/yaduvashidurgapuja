...
const QUICK_DEVOTIONS = [
  'Jai Mata Di! 🚩',
  'Jai Maa Durga! 🌸',
  'Jai Durge Durgati Nashini! ✨',
  'Pranam Maa Bhagwati 🙏',
  'Jai Kapooripur Durga Puja! 🕉️',
];
...
export const ArtiChatPanel: React.FC<ArtiChatPanelProps> = ({
  ...
  const effectiveName = isAuthenticated && user?.name ? user.name : (guestName.trim() || 'Devotee');
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <div className="text-left">
            <h3 className="text-sm font-heading font-bold text-cream-50 leading-tight">
              Live Aarti Chat
            </h3>
            <p className="text-[10px] text-gold-300/80 font-body">
              {comments.length} chants • Auto-expires in 1 hour
            </p>
          </div>
        </div>
      ...
      <AnimatePresence initial={false}>
        {isExpanded && (
          ...
          >
            ...
            {!isChatEnabled && (
              <div className="bg-red-900/10 border-b border-red-500/30 px-3 py-2 text-xs text-red-900 flex items-center gap-1.5 font-body font-semibold">
                <Lock className="w-3.5 h-3.5 text-red-700 shrink-0" />
                <span>Chat is temporarily disabled by administrator.</span>
              </div>
            )}

            {/* Rate Limit Warning Banner */}
            {rateLimitWarning && (
              <div className="bg-amber-100 border-b border-amber-300 px-3 py-1.5 text-xs text-amber-900 flex items-center gap-1.5 font-body animate-shake">
            ...
            <div className="flex-1 p-3.5 overflow-y-auto space-y-2.5 bg-cream-50/80">
              {isLoading ? (
                <div className="h-full flex items-center justify-center text-muted text-xs font-body">
                ...
                <div className="h-full flex flex-col items-center justify-center text-center p-6 text-muted space-y-2">
                  ...
                  <p className="text-xs font-body text-dark-800 font-semibold">
                    No chants posted yet.
                  </p>
                  <p className="text-[11px] text-muted font-body">
                    Be the first to chant Jai Mata Di in the live aarti! 🙏
                  </p>
                </div>
              ) : (
                comments.map((cmt, idx) => (
                  <motion.div
                    ...
                    className="p-2.5 rounded-xl bg-cream-100/90 border border-cream-300/80 shadow-xs text-xs font-body"
                  >
            ...
            <div className="px-3 py-2 bg-cream-100 border-t border-cream-200 flex items-center gap-1.5 overflow-x-auto no-scrollbar">
              {QUICK_DEVOTIONS.map((phrase, idx) => (
                <button
                  ...
                  className="shrink-0 px-2.5 py-1 rounded-full bg-cream-50 hover:bg-gold-500/15 text-maroon-900 text-[11px] font-semibold border border-cream-300 hover:border-gold-500/40 disabled:opacity-40 disabled:cursor-not-allowed transition-colors font-body"
                >
                  {phrase}
                </button>
              ))}
            ...
            <div className="p-3 bg-cream-100 border-t border-cream-300 space-y-2">
              ...
              {!isAuthenticated && isChatEnabled && (
                <div className="flex items-center justify-between text-[11px] text-muted font-body px-1">
                  <span>
                    Posting as:{' '}
                    <strong className="text-maroon-900 font-semibold">
                      {guestName.trim() || 'Devotee (Guest)'}
                    </strong>
                  </span>
                  <button
                    type="button"
                  ...
                  >
                    {showNameField ? 'Hide' : 'Change Name'}
                  </button>
                </div>
              )}
              ...
              <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                <div className="relative flex-1">
                  <input
                    ...
                    placeholder={
                      isChatEnabled
                        ? 'Type a chant or message for Maa Durga...'
                        : 'Live chat is currently disabled...'
                    }
                    value={message}
                    onChange={(e) => setMessage(e.target.value)}
                    maxLength={200}
                    className="w-full pl-3 pr-12 py-2 rounded-xl border border-cream-300 bg-cream-50 text-dark-900 text-xs font-body focus:outline-none focus:ring-2 focus:ring-maroon-700/20 focus:border-maroon-700 disabled:opacity-50 disabled:bg-cream-200/50"
                  />
                  {isChatEnabled && (
